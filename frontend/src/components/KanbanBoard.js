...
const STATUSES = [
  ...
  { key: 'in_progress', label: 'В работе', color: 'yellow', includeStatuses: ['in_progress', 'new'] },
  { key: 'delivery', label: 'Доставка', color: 'blue' },
  { key: 'declined', label: 'Отклонено', color: 'red' },
];

export default function KanbanBoard({ leads, onSelect }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-8">
      <div className="flex justify-between items-center border-b pb-4 mb-6">
        <h1 className="text-2xl font-bold text-blue-600">CRM Система</h1>
        {/* Можно добавить "Последнее обновление" */}
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {STATUSES.map(({ key, label, color }) => (
          <div key={key} className={`bg-${color}-50 rounded-lg p-4 border border-${color}-200 status-column`}>
            <div className={`text-lg font-semibold text-${color}-700 mb-2`}>{label}</div>
